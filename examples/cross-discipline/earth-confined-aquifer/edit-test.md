...
1. Imported a local copy of the canonical SVG; 30 semantic objects were available.
2. Changed live `figure-title` to `Confined aquifer: static head` using the text control.
3. Recoloured `aquifer` to `#4c99bb` using the colour control.
4. Moved `flow-0` by x=0, y=8 SVG units using the move controls.
