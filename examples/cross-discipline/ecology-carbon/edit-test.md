...
1. Imported a local copy of the canonical SVG; 51 semantic objects were available.
2. Changed live `figure-title` to `Selected terrestrial carbon pathways` using the text control.
3. Recoloured `leaf-0` to `#5e8955` using the colour control.
4. Moved `herbivory` by x=0, y=8 SVG units using the move controls.
