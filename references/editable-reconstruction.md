...
4. Refine edges, occlusion, line weights, arrowheads and live-text spacing. Group movement must keep arrow shafts/heads and component shading together. Recolouring should retain intentional opacity and relative face shading; verify it on the actual component. Use [editorial refinement](editorial-refinement.md) for typography, useful density and quality-based common-master selection.
...
Component recolouring clones referenced gradients and approximately preserves relative face tones, offsets and opacity. It supports explicit opaque three/six-digit hex paints and gradient stops. Unsupported paints or inherited/CSS gradient colours are retained with a status notice; CSS-defined object paints are outside this basic editing scope. It does not alter mask/clip definitions or promise physical/perceptual colour fidelity. Check the render and use a full editor for complex styles.
